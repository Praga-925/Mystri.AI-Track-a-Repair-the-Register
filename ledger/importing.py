...
def import_csv(db, text, kind):
    ...
    with db:
        for line, row in enumerate(reader, 2):
            try:
                normalized_row = normalize(row, kind, customers)
                if kind == 'invoices':
                    outcome = insert_invoice(db, normalized_row)
                else:
                    outcome = insert_payment(db, normalized_row, find_invoice(db, normalized_row))
                result[outcome] += 1
